...
export default function Loads() {
  ...
  async function postLoad() {
    setMsg("");
    if (!user) { setMsg("❌ Login required to post loads."); return; }
    ...
    const insert = {
      ...
      rpm,
      budget,
      in_house_first: !!draft.in_house_first,
      created_by: user.id, // checked by RLS policy
    };
  ...
  async function claimLoad(id: string) {
    setMsg("");
    if (!user) { setMsg("❌ Login required to claim loads."); return; }
    const { data, error } = await supabase
      .from("loads")
      .update({ claimed_by_user: user.id })
      .eq("id", id)
      .is("claimed_by_user", null)         // only if unclaimed
      .select("*")
      .single();

    if (error) { setMsg("❌ " + error.message); return; }
    if (data) {
      setLoads(prev => prev.map(l => (l.id === id ? (data as Load) : l)));
      setMsg("✅ Load claimed.");
    } else {
                ...
                <div key={l.id} className="load-row">
                  ...
                  <div style={{display:"flex",gap:8,justifyContent:"flex-end"}}>
                    ...
                    {l.claimed_by_user ? (
                      ...
                      <button onClick={() => claimLoad(l.id)} disabled={!user}>Claim</button>
                    )}
                  </div>
                </div>
              );
